...
public class ObjectArrayEx16 {
	...
	public static void main(String[] args) {
		...
		hMap = null; // HashMap의 자체를 삭제
		
		
		// 웹에서 자주 사용하는 예시 1
		
		TempMrmber temp = new TempMrmber();
		temp.id = "user1";
		temp.name = "clxm";
		temp.passwd = "1";
		temp.email = "wwnsg@gmail.com";
		temp.address = "서울";
		
		HashMap<String, Object> tempMap = new HashMap<String, Object>();
		tempMap.put("id", "faaad");
		tempMap.put("name", "유저2");
		tempMap.put("passwd", "2");
		tempMap.put("email", "gjwns4924@gmail.com");
		tempMap.put("address", "인천");
		// class 외에 추가적인 데이터 저장이 가능
		...
		// 웹에서 자주 사용하는 예시 2
		HashMap<String, TempMrmber> memberMap = new HashMap<String, TempMrmber>();
		
		// 실습 데이터 생성

		for (int i = 0; i < 101; i++) {
			
			TempMrmber tmp = new TempMrmber();
			tmp.id = "user" + i;
			tmp.passwd = "0000";
			tmp.name = "익명";
		...
		testMap.put("키4", 3.14);
		testMap.put("키5", 'p');
		testMap.put("키6", new TempMrmber());
		testMap.put("키7", new Object());
	}
